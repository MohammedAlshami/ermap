...
const CHART_HUE = '#C5D86D';
const CHART_BG = '#141414';
const CHART_TEXT = '#e5e7eb';
const CHART_GRID = 'rgba(255,255,255,0.06)';
const CHART_BORDER = 'rgba(255,255,255,0.1)';
...
export function StockChart({ symbol, className = '', height = 380, range = '3mo' }: StockChartProps) {
  ...
  useEffect(() => {
    ...
    const areaSeries = chart.addSeries(AreaSeries, {
      topColor: CHART_HUE,
      bottomColor: 'rgba(197, 216, 109, 0.12)',
      lineColor: CHART_HUE,
      lineWidth: 2,
    ...
    legend.style.cssText =
      'position: absolute; left: 12px; top: 12px; z-index: 10; font-size: 14px; font-family: inherit; line-height: 1.4; font-weight: 300; color: #e5e7eb; pointer-events: none;';
    container.appendChild(legend);
    legendRef.current = legend;

    const setLegendHtml = (name: string, date: string, price: string) => {
      legend.innerHTML = `<div style="font-size: 18px; font-weight: 600; margin-bottom: 4px; color: #f3f4f6;">${name}</div><div style="font-size: 20px; font-weight: 600; margin-bottom: 2px; color: ${CHART_HUE};">${price}</div><div style="font-size: 12px; color: #9ca3af;">${date}</div>`;
    };
  ...
  return (
    <div className={`rounded-lg border border-border/50 overflow-hidden ${className}`} style={{ backgroundColor: CHART_BG }}>
