...
const MODEL_PATH = path.join(process.cwd(), 'models', 'building_segmentation.onnx');

export interface DetectResult {
  mask: number[];
  width: number;
  height: number;
  /** Optional: PNG overlay image as base64 for map display */
...
export class BuildingService {
    ...
    this.initPromise = (async () => {
      if (!fs.existsSync(MODEL_PATH)) {
        throw new ServiceUnavailableException(
          `Model not found at ${MODEL_PATH}. Add building_segmentation.onnx (see backend/models/README.md).`,
        );
      }
  ...
  /** Convert RGB buffer to NCHW float32 (1, 3, H, W), normalized 0-1 */
  ...
  async detectBuildings(imageBuffer: Buffer): Promise<DetectResult> {
  ...
  /** Create a semi-transparent PNG overlay (buildings in blue) for map display */
