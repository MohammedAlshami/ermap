...
export interface SidePanelConfig {
  ...
  content?: ReactNode;
  className?: string;
  /** When set, shows a back button to the left of the title; called on click */
  onBack?: () => void;
  /** Optional content to render on the right side of the header (e.g. share button) */
...
/**
 * Position of slide content (title + description) on the map – 3×3 grid
 */
export type SlideContentPosition =
  | 'top-left'
  | 'top-center'
  | 'top-right'
  | 'middle-left'
...
export interface SlideConfig {
  ...
  title: string;
  description?: string;
  /** Where to show title + description on the map (3×3 grid). Default: bottom-center */
